...
export class AnalyticsService {
  ...
  async generateOverviewAnalytics(loans: LoanModel[]): Promise<OverviewAnalytics> {
    try {
      this.logger.info('Generating overview analytics', { loanCount: loans.length });
      ...
      const totalDebt = loans.reduce((sum, loan) => sum + loan.calculateTotalWithInterest(), 0);
      ...
      const smallestLoan = loanAmounts.length > 0 ? Math.min(...loanAmounts) : 0;
      ...
      this.logger.error('Failed to generate overview analytics', error as Error);
      throw error;
    }
  }

  async generateMonthlyBreakdown(loans: LoanModel[]): Promise<MonthlyBreakdown[]> {
    try {
      ...
      const breakdown: MonthlyBreakdown[] = months.map(month => {
        ...
        return {
          ...
          averageAmount: data.newLoans > 0 ? data.totalAmount / data.newLoans : 0
        };
      });

      this.logger.info('Monthly breakdown generated successfully', { months: breakdown.length });
      return breakdown;
  ...
  async generateLenderAnalysis(loans: LoanModel[]): Promise<LenderAnalysis[]> {
    try {
      ...
      const analysis: LenderAnalysis[] = Array.from(lenderMap.entries()).map(([name, data]) => {
        const totalLoans = data.loans.length;
        const paymentRate = totalLoans > 0 ? (data.paidLoans / totalLoans) * 100 : 0;
        const averageAmount = totalLoans > 0 ? data.totalAmount / totalLoans : 0;
        
        // Calculate risk level based on payment rate and overdue loans
        let riskLevel: 'Low' | 'Medium' | 'High' = 'Low';
        const overdueRate = totalLoans > 0 ? (data.overdueLoans / totalLoans) * 100 : 0;
      ...
      this.logger.info('Lender analysis generated successfully', { lenders: analysis.length });
      return analysis;
  ...
  async generateInterestAnalysis(loans: LoanModel[]): Promise<InterestAnalysis> {
    try {
      this.logger.info('Generating interest analysis');

      const loansWithInterest = loans.filter(loan => loan.interestRate && loan.interestRate > 0);
      const totalInterest = loans.reduce((sum, loan) => {
        if (loan.interestRate) {
          return sum + (loan.amount * loan.interestRate / 100);
        }
      ...
      const interestPercentage = totalPrincipal > 0 ? (totalInterest / totalPrincipal) * 100 : 0;
      ...
      const highestRate = interestRates.length > 0 ? Math.max(...interestRates) : 0;
      const lowestRate = interestRates.length > 0 ? Math.min(...interestRates) : 0;

      const interestLoanPercentage = loans.length > 0 ? 
        (loansWithInterest.length / loans.length) * 100 : 0;

      // Calculate potential revenue if all loans were paid with interest
      const potentialRevenue = loans.reduce((sum, loan) => sum + loan.calculateTotalWithInterest(), 0);
  ...
  async generatePaymentTrends(loans: LoanModel[]): Promise<PaymentTrend[]> {
    try {
      ...
      const periods = ['Last 7 days', 'Last 30 days', 'Last 90 days', 'Last 6 months', 'Last year'];
      ...
      for (let i = 0; i < periods.length; i++) {
        ...
        const amount = periodLoans.reduce((sum, loan) => sum + loan.calculateTotalWithInterest(), 0);
        ...
        if (i > 0) {
          ...
          const prevAmount = prevPeriodLoans.reduce((sum, loan) => sum + loan.calculateTotalWithInterest(), 0);
      ...
      this.logger.info('Payment trends generated successfully', { trends: trends.length });
      return trends;
  ...
  async generateRiskAssessment(loans: LoanModel[]): Promise<RiskAssessment> {
    try {
      ...
      const overdueLoans = loans.filter(loan => loan.isOverdue()).length;
      const overdueRate = loans.length > 0 ? (overdueLoans / loans.length) * 100 : 0;
      ...
      loans.forEach(loan => {
        ...
        lenderMap.set(loan.lenderName, current + loan.calculateTotalWithInterest());
      });

      const totalDebt = loans.reduce((sum, loan) => sum + loan.calculateTotalWithInterest(), 0);
      const maxLenderExposure = Math.max(...Array.from(lenderMap.values()));
      const concentrationRate = totalDebt > 0 ? (maxLenderExposure / totalDebt) * 100 : 0;
      const concentrationScore = concentrationRate > 50 ? 80 : concentrationRate > 30 ? 50 : 20;

      factors.push({
        name: 'Concentration Risk',
        score: concentrationScore,
        impact: concentrationRate > 50 ? 'High' : concentrationRate > 30 ? 'Medium' : 'Low',
        recommendation: concentrationRate > 50 ?
      ...
      const loansWithInterest = loans.filter(loan => loan.interestRate && loan.interestRate > 0);
      const noInterestRate = loans.length > 0 ? ((loans.length - loansWithInterest.length) / loans.length) * 100 : 0;
      const interestRiskScore = noInterestRate > 70 ? 60 : noInterestRate > 40 ? 30 : 10;

      factors.push({
        name: 'Interest Rate Risk',
        score: interestRiskScore,
        impact: noInterestRate > 70 ? 'Medium' : noInterestRate > 40 ? 'Low' : 'Very Low',
        recommendation: noInterestRate > 70 ?
      ...
      const paidLoans = loans.filter(loan => loan.isPaid).length;
      const paymentRate = loans.length > 0 ? (paidLoans / loans.length) * 100 : 0;
      ...
      factors.push({
        name: 'Portfolio Size',
        score: sizeScore,
        impact: portfolioSize < 5 ? 'Medium' : portfolioSize < 10 ? 'Low' : 'Very Low',
        recommendation: portfolioSize < 5 ?
      ...
      if (totalRiskScore >= 70) {
        summary = 'High risk portfolio requiring immediate attention. Focus on overdue collections and risk mitigation strategies.';
      } else if (totalRiskScore >= 40) {
        summary = 'Moderate risk portfolio. Monitor key metrics closely and implement preventive measures.';
      } else {
        summary = 'Low risk portfolio with good fundamentals. Maintain current practices and continue monitoring.';
      }
      ...
      this.logger.info('Risk assessment generated successfully', { overallRisk: assessment.overallRisk });
      return assessment;
  ...
  async calculatePortfolioMetrics(loans: LoanModel[]): Promise<{
    ...
    try {
      const totalValue = loans.reduce((sum, loan) => sum + loan.calculateTotalWithInterest(), 0);
      const totalPrincipal = loans.reduce((sum, loan) => sum + loan.amount, 0);
      ...
      const portfolioYield = totalPrincipal > 0 ? (totalInterest / totalPrincipal) * 100 : 0;
      ...
      const paidLoans = loans.filter(loan => loan.isPaid).length;
      const recoveryRate = loans.length > 0 ? (paidLoans / loans.length) * 100 : 0;
  ...
  async generateCashFlowProjection(loans: LoanModel[], months: number = 12): Promise<Array<{
    month: string;
    ...
    try {
      ...
      this.logger.error('Failed to generate cash flow projection', error as Error);
      throw error;
    }
  }
  ...
  async generatePerformanceComparison(loans: LoanModel[]): Promise<{
    ...
    try {
      ...
      const previousMetrics = await this.calculatePortfolioMetrics(previousLoans);

      const comparison = {
        volumeChange: this.calculatePercentageChange(currentLoans.length, previousLoans.length),
        valueChange: this.calculatePercentageChange(currentMetrics.totalValue, previousMetrics.totalValue),
        yieldChange: this.calculatePercentageChange(currentMetrics.portfolioYield, previousMetrics.portfolioYield),
        defaultRateChange: this.calculatePercentageChange(currentMetrics.defaultRate, previousMetrics.defaultRate),
        recoveryRateChange: this.calculatePercentageChange(currentMetrics.recoveryRate, previousMetrics.recoveryRate)
      };
      ...
      this.logger.error('Failed to generate performance comparison', error as Error);
      throw error;
    }
  }
  ...
  async exportAnalyticsData(loans: LoanModel[]): Promise<{
    ...
    try {
      ...
      const [overview, monthly, lenders, interest, trends, risk] = await Promise.all([
